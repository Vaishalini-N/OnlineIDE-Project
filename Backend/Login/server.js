...
// Create basic middleware if files are missing
const verifyRecaptcha = (req, res, next) => {
  console.log('✅ reCAPTCHA verification bypassed for testing');
  next();
};

const cleanExpired = (req, res, next) => {
  console.log('✅ Clean expired middleware bypassed');
  next();
};
...
const logUserAction = async (user, action) => {
  console.log(`User action: ${user.username} - ${action}`);
};

const updateLanguageCount = (user, countType, language) => {
  if (!user[countType]) user[countType] = {};
  user[countType][language] = (user[countType][language] || 0) + 1;
  return true;
};

// Basic email functions (placeholder)
const sendOtpEmail = async (email, otp) => {
  console.log(`📧 OTP ${otp} sent to ${email}`);
  return true;
};

const sendDelEmail = async (email) => {
  console.log(`📧 Account deletion email sent to ${email}`);
  return true;
};

const sendPassChangeEmail = async (email) => {
  console.log(`📧 Password change email sent to ${email}`);
  return true;
};

const sendUsernameChangeEmail = async (email, oldUsername, newUsername) => {
  console.log(`📧 Username change email sent to ${email}: ${oldUsername} -> ${newUsername}`);
  return true;
};
...
app.post('/api/register', verifyRecaptcha, cleanExpired, async (req, res) => {
	...
	try {
		...
		if (!usernameRegex.test(username)) {
			return res.status(400).json({ msg: 'Username can only contain letters, numbers, underscores, hyphens, and periods (5-30 characters).' });
		}

		if (username.length < 5 || username.length > 30) {
			return res.status(400).json({ msg: 'Username should be between 5 and 30 characters' });
		}
		...
		if (!pwdRegex.test(password)) {
			return res.status(400).json({ msg: "Password must be at least 8 characters and contain only ASCII characters." });
		}
...
app.post('/api/login',  async (req, res) => {
	...
	try {
		...
		if (user.googleId && !user.password) {
			return res.status(403).json({ msg: "Login with Google" });
		}
...
app.post('/api/auth/google', verifyRecaptcha, async (req, res) => {
  ...
  try {
    ...
    if (!user) {
		...
		while (await User.exists({ username: finalUsername }) || reservedUsernames.includes(finalUsername)) {
			const randomSuffix = Math.random().toString(36).substring(2, 6);
			finalUsername = `${baseUsername}_${randomSuffix}`;
			tries++;
			if (tries > 10) {
				return res.status(500).json({ message: 'Unable to generate unique username.' });
			}
		}
